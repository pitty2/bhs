...
DECLARE_DATA_IDENT(logical_or_op_id,	"||");
DECLARE_DATA_FUNC(logical_or_op_fnc,	&logical_or_op_id,	logical_or_fnc);
/*
	-----------------------------------------------------------------------
 */
int
logical_or_fnc(				/* logical OR (||) */
	...
	&&	(comp_id_cmp(DID(arg2), BOOLEAN) == DATA_EQ)
		...
		b =  (DBOOL(arg1) == DBOOL(arg2)) ?
			 ((DBOOL(arg1) == TRUE) ? TRUE : FALSE) : TRUE;
