...
DECLARE_DATA_IDENT(logical_and_op_id,	"&&");
DECLARE_DATA_FUNC(logical_and_op_fnc,	&logical_and_op_id,	logical_and_fnc);
/*
	-----------------------------------------------------------------------
 */
int
logical_and_fnc(			/* logical AND (&&) */
	...
	if(
		(comp_id_cmp(DID(arg1), BOOLEAN) == DATA_EQ) 
	&&	(comp_id_cmp(DID(arg2), BOOLEAN) == DATA_EQ)
	) {
		bool_t	b;

		b = (DBOOL(arg1) == DBOOL(arg2)) ? 
			((DBOOL(arg1) == TRUE) ? TRUE : FALSE) : FALSE;
