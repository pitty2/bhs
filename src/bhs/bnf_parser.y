...
%parse-param	{ fncrc_t *fncrc }
%parse-param	{ sess_t *sp }
%parse-param	{ comp_t *cp }

%lex-param	{ fncrc_t *fncrc }
%lex-param	{ sess_t *sp }
%lex-param	{ comp_t *cp }
...
%{
#define YYLEX_PARAM fncrc, sp, cp, cp->comp_scanner_db
void	yyerror (fncrc_t *fncrc, sess_t *sp, comp_t *cp, char const *msg);
int	bnf_expr_reduce(fncrc_t *fncrc, sess_t *sp, comp_t *cp, data_t *expr, data_t **pp);
%}

%token	<dp>	LEX_FATAL_ERROR
%destructor	{ data_drop (fncrc, sp, $$); }	BNF_INCLUDE_TOK
%destructor	{ data_drop (fncrc, sp, $$); }	BNF_FILENAME_TOK
%token	<dp>	BNF_INCLUDE_TOK
%token	<dp>	BNF_FILENAME_TOK

%destructor	{ data_drop (fncrc, sp, $$); }	BNF_ID_TOK
%destructor	{ data_drop (fncrc, sp, $$); }	BNF_STRING_TOK
%destructor	{ data_drop (fncrc, sp, $$); }	BNF_CHAR_TOK
...
%destructor	{ data_drop (fncrc, sp, $$); }	'?'
%destructor	{ data_drop (fncrc, sp, $$); }	BNF_DATE_BUILTIN_TOK
%destructor	{ data_drop (fncrc, sp, $$); }	BNF_ENV_BUILTIN_TOK
%destructor	{ data_drop (fncrc, sp, $$); }	BNF_LEN_BUILTIN_TOK
%destructor	{ data_drop (fncrc, sp, $$); }	BNF_PWD_BUILTIN_TOK
%destructor	{ data_drop (fncrc, sp, $$); }	BNF_UNAME_BUILTIN_TOK
%token	<dp>	ID_TOK
%token	<dp>	OBJ_ID_TOK
%token	<dp>	STRING_TOK
%token	<dp>	CHAR_TOK
...
%token	<dp>	BNF_ASGN_SIGN
%destructor	{ data_drop (fncrc, sp, $$); }	BNF_ASGN_SIGN
...
assign		: name BNF_ASGN_SIGN value ';'
		{
...
expr		: _expr
		{
			YY_TRY(bnf_expr_reduce, sp, cp, $1, &($$));
		}
		| bool_expr
		{
			$$ = $1;
